...
\usepackage{xspace}
\usepackage{ragged2e}
\usepackage[palette=$if(colorPalette)$$colorPalette$$else$default$endif$]{\packageDir/mycolors}
...
$if(abstract)$
    \newcommand{\QuetzalAbstract}{{$abstract$}}
$endif$
...
$if(titlepage)$
    \usepackage[%
    $if(titlepageAbstract)$ abstract $endif$%
    ]{\packageDir/titlepageoptions}
    $if(reportNo)$
        \renewcommand\QuetzalTitlePageReportNo{$reportNo$}
    $endif$
    
    $if(institution)$
        \renewcommand\QuetzalTitlePageInstitution{$institution$}
    $endif$
    
    $if(logo)$
        \renewcommand\QuetzalTitlePageLogo{$logo$}
    $endif$
    
    $if(logoWidth)$
        \renewcommand\QuetzalTitlePageLogoWidth{$logoWidth$}
    $endif$
    
    $if(titlepageTextColor)$
        \renewcommand\QuetzalTitlePageTextColor{$titlepageTextColor$}
    $endif$
    $if(titlepageAbstract)$
        \renewcommand\QuetzalTitlePageAbstract{1}
    $else$
        \renewcommand\QuetzalTitlePageAbstract{0}
    $endif$
$else$
    % \usepackage[runin]{abstract}
$endif$
...
$if(titleFormat)$
\usepackage[style=$titleFormat$]{\packageDir/title}
$elseif(titlepage)$
$else$
...
% Necessary to let pandoc use images.
\makeatletter
...
% $if(abstract)$  % Maybe I could get rid of this?
...
$if(biblatex)$
\printbibliography$if(biblioTitle)$[title=$biblioTitle$]$endif$
$endif$
